...
export const emitirCertificadoSuciveDataUseCase = async (currentUser: any, params: { vehicleData: ConsultarDeudaData, requestNumber: string }): Promise<IGenerateAndSaveScrappedDataRes<EmitirCertificadoSuciveDataRes>> => {
    ...
    const imagePathsUrls = imagePaths.map((image, index) => saveImage(image, `${userId}_${vehicleData.matricula}_${vehicleData.padron}_certificado_sucive_${requestNumber}_${index}.png`));
    const pdfPathsUrls = pdfPaths.map((pdf, index) => savePdf(pdf, `${userId}_${vehicleData.matricula}_${vehicleData.padron}_certificado_sucive_${requestNumber}_${index}.pdf`));
